...
func TestLogger_Formatting(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			msg:      "hello",
			fields:   []any{String("long", strings.Repeat("a", 50))},
...
func TestLogger_OmitFields(t *testing.T) {
	...
	t.Run("standard_format_omitting_fields", func(t *testing.T) {
		...
		l := New(cfg)
		l.Info("testing", String("account", "lemon4ksan"), Int("steam_id", 12345), String("keep", "me"))
		err := l.Close()
		require.NoError(t, err)
...
func TestLogger_Any(t *testing.T) {
	t.Parallel()
	...
	l.Info("testing",
		...
		Int("key_field", 42),
		123, "val_for_int_key",
		slog.String("key_slog_attr", "value_slog_attr"),
		[]Field{String("key_slice_field", "value_slice_field")},
		[]slog.Attr{slog.Int("key_slice_slog", 100)},
		[]any{"key_nested", "value_nested"},
		"lone_key", // odd number of arguments
	)
	...
	assert.Equal(t, "value_str", entry["key_str"])
	assert.Equal(t, float64(42), entry["key_field"])
	assert.Equal(t, "val_for_int_key", entry["123"])
	assert.Equal(t, "value_slog_attr", entry["key_slog_attr"])
	assert.Equal(t, "value_slice_field", entry["key_slice_field"])
	assert.Equal(t, float64(100), entry["key_slice_slog"])
	assert.Equal(t, "value_nested", entry["key_nested"])
...
func BenchmarkLogger_Fields(b *testing.B) {
	...
	for b.Loop() {
		l.Info("message", String("k1", "v1"), Int("k2", 42))
	}
}

func BenchmarkLogger_Any(b *testing.B) {
	cfg := DefaultConfig(LevelInfo)
	cfg.Output = io.Discard
