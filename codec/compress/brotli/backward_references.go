...
func computeDistanceCode(distance, max_distance uint, dist_cache []int) uint {
	if distance <= max_distance {
		distance_plus_3 := distance + 3
		offset0 := distance_plus_3 - uint(dist_cache[0])
		offset1 := distance_plus_3 - uint(dist_cache[1])
...
func createBackwardReferences(
	...
	random_heuristics_window_size := literalSpreeLengthForSparseSearch(params)
	...
	for position+hasher.HashTypeLength() < pos_end {
		...
		if sr.score > kMinScore {
			...
			max_distance = brotli_min_size_t(position, max_backward_limit)
			{
				...
				distance_code := computeDistanceCode(sr.distance, max_distance+gap, dist_cache)
			...
			*num_literals += insert_length
			...
			{
				range_start := position + 2
				range_end := brotli_min_size_t(position+sr.len, store_end)
			...
			if position > apply_random_heuristics {
				...
				if position > apply_random_heuristics+4*random_heuristics_window_size {
					kMargin := brotli_max_size_t(hasher.StoreLookahead()-1, 4)
					...
					pos_jump := brotli_min_size_t(position+16, pos_end-kMargin)
					for ; position < pos_jump; position += 4 {
						hasher.Store(ringbuffer, ringbuffer_mask, position)
						insert_length += 4
					}
				} else {
					kMargin := brotli_max_size_t(hasher.StoreLookahead()-1, 2)
					pos_jump := brotli_min_size_t(position+8, pos_end-kMargin)
