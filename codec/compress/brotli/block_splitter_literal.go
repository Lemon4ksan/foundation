...
func findBlocksLiteral(
	...
	data_size := histogramDataSizeLiteral()
...
func clusterBlocksLiteral(data []byte, length, num_blocks uint, block_ids []byte, split *blockSplit) {
	...
	all_histograms := make([]histogramLiteral, all_histograms_capacity)
	...
	histograms := make([]histogramLiteral, brotli_min_size_t(num_blocks, histogramsPerBatch))
	...
	pos = 0
	{
		...
		for i = 0; i < num_blocks; i++ {
			...
			for j = 0; j < num_final_clusters; j++ {
				cur_bits := histogramBitCostDistanceLiteral(&histo, &all_histograms[clusters[j]])
