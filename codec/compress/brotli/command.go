...
func getLengthCode(insertlen, copylen uint, use_last_distance bool, code *uint16) {
	...
	copycode := getCopyLengthCode(copylen)
...
func makeCommand(
	...
	delta := uint32(byte(int8(copylen_code_delta)))
...
func commandRestoreDistanceCode(self *command, dist *distanceParams) uint32 {
	if uint32(self.dist_prefix_&0x3FF) < numDistanceShortCodes+dist.num_direct_distance_codes {
		...
		dcode := uint32(self.dist_prefix_) & 0x3FF
		nbits := uint32(self.dist_prefix_) >> 10
		extra := self.dist_extra_
		var postfix_mask uint32 = (1 << dist.distance_postfix_bits) - 1
		hcode := (dcode - dist.num_direct_distance_codes - numDistanceShortCodes) >> dist.distance_postfix_bits
		lcode := (dcode - dist.num_direct_distance_codes - numDistanceShortCodes) & postfix_mask
...
func commandDistanceContext(self *command) uint32 {
	r := uint32(self.cmd_prefix_) >> 6
	c := uint32(self.cmd_prefix_) & 7
...
func commandCopyLenCode(self *command) uint32 {
	modifier := self.copy_len_ >> 25
	delta := int32(int8(byte(modifier | (modifier&0x40)<<1)))
