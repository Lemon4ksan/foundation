...
func hash(data []byte) uint32 {
	h := binary.LittleEndian.Uint32(data) * kDictHashMul32
...
func isMatch(d *dictionary, w dictWord, data []byte, max_length uint) bool {
	if uint(w.len) > max_length {
		...
		offset := uint(d.offsets_by_length[w.len]) + uint(w.len)*uint(w.idx)
		dict := d.data[offset:]
		switch w.transform {
		case 0:
...
func findAllStaticDictionaryMatches(
	...
	{
		offset := uint(dict.buckets[hash(data)])
		end := offset == 0
		for !end {
			w := dict.dict_words[offset]
			offset++
			l := uint(w.len) & 0x1F
			n := uint(1) << dict.words.size_bits_by_length[l]
			id := uint(w.idx)
			end = w.len&0x80 != 0
			w.len = byte(l)
			if w.transform == 0 {
				matchlen := dictMatchLength(dict.words, data, id, l, max_length)
				var s []byte
				var minlen uint
				var maxlen uint
				var len uint
				...
				for len = minlen; len <= maxlen; len++ {
					cut := l - len
					transform_id := (cut << 2) + uint((dict.cutoffTransforms>>(cut*6))&0x3F)
				...
				is_all_caps := (w.transform != transformUppercaseFirst)
	...
	if max_length >= 5 && (data[0] == ' ' || data[0] == '.') {
		is_space := (data[0] == ' ')
		offset := uint(dict.buckets[hash(data[1:])])
		end := offset == 0
		for !end {
			w := dict.dict_words[offset]
			offset++
	...
	if max_length >= 6 {
		...
		if (data[1] == ' ' && (data[0] == 'e' || data[0] == 's' || data[0] == ',')) ||
			...
			offset := uint(dict.buckets[hash(data[2:])])
			end := offset == 0
			for !end {
				w := dict.dict_words[offset]
				offset++
	...
	if max_length >= 9 {
		...
		if (data[0] == ' ' && data[1] == 't' && data[2] == 'h' && data[3] == 'e' && data[4] == ' ') ||
			...
			offset := uint(dict.buckets[hash(data[5:])])
			end := offset == 0
			for !end {
				w := dict.dict_words[offset]
				offset++
