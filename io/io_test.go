...
func TestDecompressReadCloser(t *testing.T) {
	...
	rc := &DecompressReadCloser{
		Reader: gr,
		Closer: stdio.NopCloser(nil),
	}

	out, err := stdio.ReadAll(rc)
	require.NoError(t, err)
...
func TestLimitCheckingReadCloser(t *testing.T) {
	...
	inner := stdio.NopCloser(strings.NewReader("1234567890"))
