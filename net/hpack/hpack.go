...
func (hp *HPACK) decodeIndexed(hf *HeaderField, b []byte) ([]byte, error) {
	...
	if n == 0 {
		...
		return b, errors.New("index value of 0 is forbidden (RFC 7541 §6.1)")
	}

	hf2 := hp.peek(n)
	if hf2 == nil {
		return b, fmt.Errorf("index field not found: %d (RFC 7541 §2.3.3)", n)
	}
...
func (hp *HPACK) decodeLiteralIndexed(hf *HeaderField, b []byte) ([]byte, error) {
	...
	if c != 64 {
		...
		hf2 := hp.peek(n)
		if hf2 == nil {
			return b, fmt.Errorf("literal indexed field not found: %d", n)
		}
...
func (hp *HPACK) decodeLiteralNoIndex(hf *HeaderField, b []byte) ([]byte, error) {
	...
	if c&15 != 0 {
		...
		hf2 := hp.peek(n)
		if hf2 == nil {
			return b, fmt.Errorf("non-indexed field not found: %d", n)
		}
...
func readString(dst, b []byte) ([]byte, []byte, error) {
	if len(b) == 0 {
		return b, dst, errors.New("hpack: malformed string")
	}
	...
	b, n := readInt(7, b)
	if uint64(len(b)) < n {
		return b, dst, errors.New("hpack: unexpected size")
	}
