...
func TestGlobalRand_Basic(t *testing.T) {
	...
	assert.Equal(t, uint32(0), rand.Uint32n(0))
	assert.Equal(t, uint64(0), rand.Uint64n(0))
	assert.Equal(t, 0, rand.Intn(0))
	assert.Equal(t, int64(0), rand.Int64n(0))
	...
	assert.Equal(t, time.Duration(0), rand.Jitter(0))
	jitter := rand.Jitter(500 * time.Millisecond)
	assert.True(t, jitter >= 0 && jitter < 500*time.Millisecond)

	assert.Equal(t, 100*time.Millisecond, rand.JitterRange(100*time.Millisecond, 50*time.Millisecond))
	jr := rand.JitterRange(100*time.Millisecond, 500*time.Millisecond)
...
func TestGlobalRand_PermShuffleReadString(t *testing.T) {
	...
	// Odd length Read
	bufOdd := make([]byte, 13)
	nOdd, errOdd := rand.Read(bufOdd)
	require.NoError(t, errOdd)
	require.Equal(t, 13, nOdd)

	assert.Empty(t, rand.String(0, "abc"))
	assert.Empty(t, rand.String(10, ""))
	s := rand.String(16, "abc")
	require.Len(t, s, 16)
	for _, c := range s {
		assert.Contains(t, "abc", string(c))
	}

	an := rand.AlphaNumeric(24)
	require.Len(t, an, 24)
}
...
func TestRNG_Local(t *testing.T) {
	...
	r2 := rand.NewRNGWithSeed(42)
	require.NotNil(t, r2)
	v1 := r2.Uint64()

	r3 := rand.NewRNGWithSeed(42)
	v2 := r3.Uint64()
	assert.Equal(t, v1, v2, "Identical seed must yield identical sequence")

	var r4 rand.RNG // zero value
	...
	for g := 0; g < goroutines; g++ {
		go func() {
			...
			for i := 0; i < iterations; i++ {
				...
				_ = rand.Uint32n(1000)
				_ = rand.Uint64()
				_ = rand.Uint64n(10000)
				_ = rand.Intn(500)
				_ = rand.Int64n(5000)
				_ = rand.Float64()
				_ = rand.Jitter(time.Second)
				_ = rand.UUIDv7()
			}
		}()
	}
