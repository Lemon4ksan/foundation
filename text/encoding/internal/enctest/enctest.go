...
// Package enctest provides shared test suites and conformance harnesses for text encodings.
package enctest
...
// Transcoder represents an encoding transformer capable of byte slice and string transcoding.
...
func TestEncoding(t *testing.T, e encoding.Encoding, encoded, utf8, prefix, suffix string) {
	for _, direction := range []string{"Decode", "Encode"} {
		t.Run(fmt.Sprintf("%v/%s", e, direction), func(t *testing.T) {
			testEncodingDirection(t, e, direction, encoded, utf8, prefix, suffix)
		})
	}
}

func testEncodingDirection(t tester, e encoding.Encoding, direction, encoded, utf8, prefix, suffix string) {
...
func testFile(t tester, e encoding.Encoding) {
