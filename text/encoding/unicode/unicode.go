...
package unicode // import "github.com/lemon4ksan/foundation/text/encoding/unicode"
...
func (utf8bomEncoding) NewEncoder() *encoding.Encoder {
	return &encoding.Encoder{
		Transformer: &utf8bomEncoder{t: replaceIllFormed{}},
	}
}
...
var utf8enc = &internal.Encoding{
	Encoding: &internal.SimpleEncoding{Decoder: utf8Decoder{}, Encoder: replaceIllFormed{}},
	Name:     "UTF-8",
	MIB:      identifier.UTF8,
}
...
func (utf8Decoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	...
	for nSrc < n {
		...
		accept = acceptRanges[f>>acceptShift]
		if nSrc+size > n {
			if !atEOF {
			...
			case nSrc+1 >= n || src[nSrc+1] < accept.Lo || accept.Hi < src[nSrc+1]:
				size = 1
			case nSrc+2 >= n || src[nSrc+2] < loCB || hiCB < src[nSrc+2]:
				size = 2
			default:
		...
		} else if c = src[nSrc+2]; c < loCB || hiCB < c {
			size = 2
			goto handleInvalid // invalid continuation byte
		} else if size == 3 {
		} else if c = src[nSrc+3]; c < loCB || hiCB < c {
			size = 3
			goto handleInvalid // invalid continuation byte
		}
		nSrc += size
